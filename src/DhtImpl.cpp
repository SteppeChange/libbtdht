...
#if BT_LITTLE_ENDIAN == 1
	for(uint i=0; i!=DHT_ID_SIZE; i++) {
		b[i^3] = ((byte const*)&(id.id[0]))[i];
	}
#else
	memcpy(b, &id.id, DHT_ID_SIZE);
#endif
}
...
//			  Read32(message.transactionID.b),
	...
	if (_dht_events && message.punchType == HPTest) {
		...
		if(_my_id == to_dht && message.punchTarget_id) {
			...
			warnings_log("wrong incoming punch test to %s (my:%s)",
						 format_dht_id(to_dht).c_str(),
						 format_dht_id(_my_id).c_str());
	}
	...
	if (message.punchType == HPRelay) {
		...
		punch_request(message.punchId,
				peerID.id, punchTargetLocal, punchTargetPublic, punchTargetRelay,
					  punchExecutorId, punchExecutor);
	}
						  ...
						  std::function<void(std::string const& error_reason)> const& failed_fun)
{
	...
	CallBackPointers cbPtrs;
	cbPtrs.processListener = 0;
	// get peers in those nodes
	DhtProcessBase* p = FindNodeEventualyDhtProcess::Create(this, *dpm, target, cbPtrs
		, maxOutstanding, flags, success_fun, failed_fun);
	dpm->AddDhtProcess(p);
	dpm->Start();
}
...
DhtProcessBase* FindNodeEventualyDhtProcess::Create(DhtImpl* pDhtImpl
		, DhtProcessManager &dpm
		, const DhtID &target2
		, CallBackPointers &cbPointers
		, int maxOutstanding
		...
		, std::function<void(std::string const& error_reason)> const& a_failed_fun)
{
	...
	FindNodeEventualyDhtProcess* process = new FindNodeEventualyDhtProcess(pDhtImpl, dpm, target2
			, time(NULL), cbPointers, maxOutstanding, flags);
	process->success_fun = a_success_fun;
	process->failed_fun = a_failed_fun;
	return process;
}

FindNodeEventualyDhtProcess::FindNodeEventualyDhtProcess(DhtImpl* pDhtImpl
		, DhtProcessManager &dpm, const DhtID &target2
		, time_t startTime
		, const CallBackPointers &consumerCallbacks, int maxOutstanding
		, int flags)
		: FindNodeDhtProcess(pDhtImpl,dpm,target2,startTime
		, consumerCallbacks,maxOutstanding, flags)
{
}
...
void DhtImpl::punch_test(int punch_id, sha1_hash const& target_id, SockAddr const& target)
{
	DhtID target_dht(target_id);
	punch(HPTest, punch_id,
			&target_dht, &target, 0, 0,  // target
			0, 0,  // executor
			0); // relay node
}
...
{
	DhtID exec_dht(executor_id);
	punch(HPRelay, punch_id,
			&_my_id, &target_local, &target_public, &target_relay, // target
			&exec_dht, &executor, // executor
			&relay); // relay node
}

void DhtImpl::punch_request(int punch_id,
							DhtID const& targetId, SockAddr const& target_local, SockAddr const& target_public, SockAddr const& target_relay,
							DhtID const& executorId, SockAddr const& executor)
{
	punch(HPRequest, punch_id,
			&targetId, &target_local, &target_public, &target_relay, // target
		    &executorId, &executor,  // executor
			0); // relay node
}
					...
					SockAddr const* relay)
{
	...
	sb("d");
        ...
        if(type==HPRelay) { // executor ip/id
			if(executorId) sb("3:eid20:")(*executorId); // executor dht id
		...
		if(type==HPRequest || type==HPTest) {
			if(targetId) 	sb("3:tid20:")(*targetId); // target dht id
		}
